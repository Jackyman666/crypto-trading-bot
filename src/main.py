...
from src.utils import to_milliseconds


def run_find_signal(coin: str, amount_precision: int, price_precision: int, execute_time: int):
    """
    Worker function to run findSignal for a single coin in a thread.
    """
    try:
        print(f"Starting signal analysis for {coin}...")
        findSignal(coin, execute_time)
